...
export default function Accessibility() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-indigo-50 to-violet-50">
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-lg border-b border-stone-200 sticky top-0 z-40">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {modes.map((mode) => {
            ...
            // Map colors to static tailwind classes to prevent purging
            const colorMap: Record<string, any> = {
              blue: {
                borderActive: 'border-blue-500 shadow-blue-500/20 ring-blue-300',
                headerBg: 'from-blue-50 to-blue-100 border-blue-200',
                iconContainer: 'border-blue-200',
                icon: 'text-blue-600',
                checkBg: 'bg-blue-600',
                checkIcon: 'text-blue-600',
                btnActive: 'from-blue-600 to-blue-500 text-white shadow-blue-500/30',
                btnInactive: 'bg-blue-50 text-blue-700 hover:bg-blue-100'
              },
              purple: {
                borderActive: 'border-purple-500 shadow-purple-500/20 ring-purple-300',
                headerBg: 'from-purple-50 to-purple-100 border-purple-200',
                iconContainer: 'border-purple-200',
                icon: 'text-purple-600',
                checkBg: 'bg-purple-600',
                checkIcon: 'text-purple-600',
                btnActive: 'from-purple-600 to-purple-500 text-white shadow-purple-500/30',
                btnInactive: 'bg-purple-50 text-purple-700 hover:bg-purple-100'
              },
              green: {
                borderActive: 'border-green-500 shadow-green-500/20 ring-green-300',
                headerBg: 'from-green-50 to-green-100 border-green-200',
                iconContainer: 'border-green-200',
                icon: 'text-green-600',
                checkBg: 'bg-green-600',
                checkIcon: 'text-green-600',
                btnActive: 'from-green-600 to-green-500 text-white shadow-green-500/30',
                btnInactive: 'bg-green-50 text-green-700 hover:bg-green-100'
              },
              orange: {
                borderActive: 'border-orange-500 shadow-orange-500/20 ring-orange-300',
                headerBg: 'from-orange-50 to-orange-100 border-orange-200',
                iconContainer: 'border-orange-200',
                icon: 'text-orange-600',
                checkBg: 'bg-orange-600',
                checkIcon: 'text-orange-600',
                btnActive: 'from-orange-600 to-orange-500 text-white shadow-orange-500/30',
                btnInactive: 'bg-orange-50 text-orange-700 hover:bg-orange-100'
              },
              pink: {
                borderActive: 'border-pink-500 shadow-pink-500/20 ring-pink-300',
                headerBg: 'from-pink-50 to-pink-100 border-pink-200',
                iconContainer: 'border-pink-200',
                icon: 'text-pink-600',
                checkBg: 'bg-pink-600',
                checkIcon: 'text-pink-600',
                btnActive: 'from-pink-600 to-pink-500 text-white shadow-pink-500/30',
                btnInactive: 'bg-pink-50 text-pink-700 hover:bg-pink-100'
              }
            };
            
            const theme = colorMap[mode.color] || colorMap.blue;

            return (
              <div
                key={mode.id}
                className={`bg-white rounded-2xl shadow-lg border-2 transition-all cursor-pointer overflow-hidden flex flex-col h-full ${
                  isActive
                    ? `${theme.borderActive} ring-2`
              ...
              >
                {/* Card header */}
                <div className={`bg-gradient-to-br ${theme.headerBg} p-6 border-b`}>
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-3 bg-white rounded-xl shadow-sm border ${theme.iconContainer}`}>
                      <mode.icon className={`w-6 h-6 ${theme.icon}`} />
                    </div>
                    {/* Checkmark always rendered to prevent layout shift, toggled with opacity */}
                    <div className={`${theme.checkBg} text-white p-1.5 rounded-full transition-opacity duration-200 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                      <Check className="w-4 h-4" />
                    </div>
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 mb-2">{mode.name}</h3>
                  <p className="text-sm text-slate-600 min-h-[40px]">{mode.description}</p>
                </div>
                  ...
                  <ul className="space-y-2">
                    {mode.features.map((feature, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm text-slate-600">
                        <Check className={`w-4 h-4 mt-0.5 ${theme.checkIcon} flex-shrink-0`} />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="px-6 pb-6 mt-auto">
                  <button
                    className={`w-full py-3 rounded-xl font-medium transition-all ${
                      isActive
                        ? `bg-gradient-to-r ${theme.btnActive} shadow-lg`
                        : `${theme.btnInactive}`
                    }`}
                    onClick={(e) => {
