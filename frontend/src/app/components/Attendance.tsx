...
export default function Attendance() {
        ...
        if (data) {
          const formatted = data.map(a => {
            ...
            return {
              ...
              time: a.time_range,
              status: a.status,
              teacher: a.subjects?.teacher || 'Teacher',
              room: a.room
            };
          });
  ...
  const subjects = ['all', ...new Set(attendanceData.map(item => item.subject))];
            ...
            <div className="space-y-4">
              {filteredData.length > 0 ? filteredData.map((item, index) => {
                ...
                <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-slate-300">
                  <p className="text-slate-500 font-medium">Нет записей о посещаемости</p>
                </div>
              )}
            </div>
          </>
